...
local chunk_file = "temp"  -- Temporary filename for video chunk

-- Function to download video chunk from GitHub using wget
function downloadChunk(url, filename)
    print("Attempting to download: " .. url)
    local command = "wget " .. url .. " " .. filename  -- Simplified wget command
...
function getNextChunk()
    ...
    -- Download the chunk from GitHub and save it as "temp"
...
-- Load the video data from the temporary file
local videoData = {}
...
function processChunks()
    ...
    while true do
        ...
        -- Once all frames are processed, delete the temp file
